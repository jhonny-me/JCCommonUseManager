...
//            [newButton addTarget:self action:@selector(otherTouchesAction:) forControlEvents:UIControlEventTouchDragInside];
            ...
            newLabel.center = CGPointMake(newButton.frame.origin.x + newButton.imageView.frame.origin.x + 
                                              ...
                                              newButton.imageView.frame.size.height + newLabel.frame.size.height/2.0f + 4.0f );
...
- (void)setTitleColor:(UIColor*)normalColor selectedColor:(UIColor*)selectedColor {
    ...
    //_selectedTitleColor = [UIColor colorWithRed:255/255.0f green:255/255.0f blue:255/255.0f alpha:1.0f];
...
- (void) addBadge:(UIView*)badge atIndex:(NSInteger)index {
    ...
    badge.tag = -1;
    UIView *view = [[UIView alloc]  initWithFrame:CGRectMake((CGRectGetWidth(button.frame)/2)+10.f,3.f, 8.f, 8.f)];
    view.backgroundColor = [UIColor colorWithHexString:@"#eb5414"];
    view.layer.masksToBounds = YES;
    view.layer.cornerRadius = 4.f;
    [_badges setObject:view forKey:indexNumber];
